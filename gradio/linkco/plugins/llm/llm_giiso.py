...
def get_chat(prompt,
    ...
    # 设置代理服务器的地址和端口
    # ChatGPT API的URL
    url = "http://43.130.153.62:5502/openai"
    data = {'parameter': json.dumps(parameters), 'header': json.dumps(headers)}
    data = requests.post(url, headers=random_headers, data=data, verify=False).json()
