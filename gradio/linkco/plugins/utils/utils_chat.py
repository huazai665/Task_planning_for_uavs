...
def get_cut_history(history, cut_len: int = -1, his_len: int = -1, max_length: int = 8192):
    """
    缩减历史对话内容，减少字数消耗
...
def get_item_history(quary: str, response: str):
    """
    获取一问一答的历史对话形式
    ...
    """
    out_data = [
        {'role': 'user', 'content': quary},
        {'role': 'assistant', 'content': response}
    ]
    return out_data


def get_update_history(quary: str, response: str, history: list):
    """
    获取更新的历史对话
    ...
    Returns:
        None
    """
    history.extend(get_item_history(quary, response))
    return history
