...
from ...main import split_line_1, split_line_2
...
def get_history_prompt(history: list, role: dict = None) -> str:
    """
    获取历史对话的提示
    ...
    for idx, it in enumerate(history):
        if isinstance(it, dict):
            ...
            if idx == len(history) - 1 and role_item == 'assistant':
                prompt_str += split_line_1
...
def get_quary_prompt(quary: dict or str, response: str = '', role: dict = None) -> str:
    """
    获取当前问题的提示
    ...
    Returns:
        str: 当前问题的提示字符串
    """
    if isinstance(quary, dict):
        prompt_str = ''
        for item in quary.items():
            prompt_str += '[{}]:{}\n'.format(*item)
    else:
        prompt_str = '[user]:{}\n[assistant]:{}'.format(quary, response)
...
def get_rule_prompt(rule: list or dict or str) -> str:
    ...
    if isinstance(rule, list):
        ...
        rule_list.append('按照示例提供的形式输出结果')
        return '\n'.join(rule_list)
    else:
        return '{}\n按照示例提供的形式输出结果\n'.format(rule)


def get_chat_prompt(quary: dict or str, history: list or None, system: str or None, role: dict or None) -> str:
    """
    获取普通模型对话通用模板的提示
    ...
    his_str = get_history_prompt(history)
    quary_str = get_quary_prompt(quary)

    prompt_str = system_str + his_str + quary_str
